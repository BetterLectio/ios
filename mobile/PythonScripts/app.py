...
@app.route("/flask_live")
def flask_live():
    return "True"

@app.route("/betterlectio")
def betterlectio():
    return """
    <iframe src="https://dev--betlec.netlify.app/auth?app=iOS" style="border:0;width:100%;height:100%;"></iframe>
    """
...
def main():
    ...
    while True:
        ...
        #time.sleep(1)
        #i += 1
        resp = requests.get("http://localhost:5000/flask_live")
        if resp.text == "True":
            started = True
            print("[BACKEND LOG] Backend thread is now live at http://localhost:5000")
            break
        time.sleep(0.1)
